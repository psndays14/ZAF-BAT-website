  ...
  function init() {
    ...
    Array.prototype.forEach.call(links, function (a) {
      ...
      a.addEventListener('click', function () {
        ...
        var lbl = (a.getAttribute('aria-label') || a.textContent || '').replace(/\s+/g, ' ').trim().slice(0, 60) || 'icon';
        window.dataLayer.push({ event: 'whatsapp_click', wa_page: key, wa_label: lbl });
        if (typeof gtag === 'function') { gtag('event', 'whatsapp_click', { wa_page: key, wa_label: lbl }); }
      });
    });
  }
  if (document.readyState !== 'loading') init();
